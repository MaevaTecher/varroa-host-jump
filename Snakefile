...
rule all:
	input:
		expand("data/sketches/{sample}.msh", sample = SAMPLES)
...
rule varroaSketch:
	...
	threads: 12
	output: "data/sketches/{sample}.msh"
	shell: 
		"""
		mash sketch -p 4 -m 2 -o data/sketches/{wildcards.sample} -r  <(bowtie2 -p {threads} -x {hostBeeBowtieIndex} -1  {input.read1} -2 {input.read2}  | samtools view -S -f12 | awk '{{print ">"$1"\\n"$10}}' )
		"""
